...
The \emph{inverse} of a configuration \ilmath{\iconf{}{r} = (x, y), r \in R} is the \emph{position} of a robot. The robots move synchronously and in parallel, up to a single edge at a time. A configuration \ilmath{\conf{1}} can thus be \emph{transformed} into another configuration \ilmath{\conf{2}} in a \emph{single step} if and only if:
...
\begin{definition}\label{def:motion_planning_problem}
	Given a start configuration \ilmath{\conf{s}} and a target configuration \ilmath{\conf{t}} of a workspace, 
	the \emph{motion planning problem} asks to find a \emph{schedule} that transforms \ilmath{\conf{s}} into \ilmath{\conf{t}}.
\end{definition}

\begin{remark}\label{remark:reachability}
	For a \ilmath{2 \times 2} square and any \ilmath{1 \times n} rectangle, where \ilmath{n \geq 2}, there exist pairs of configurations that are not reachable from each other via any schedule. See \cref{fig:reachability} for an example.
