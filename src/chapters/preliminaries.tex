...
Label the vertices as $V = \set{1, 2, \dots , n_1} \times \set{1, 2, \dots , n_2}$, and denote their $x$ and $y$ -coordinates as \fda{I think ``by'' is better than ``as''} $v_x$ and $v_y$ respectively for any vertex $v \in V$.
To measure distance, we use the \emph{Manhattan norm} $L_1 = \manhattan{v - w} \coloneqq (\abs{v_x - w_x} + \abs{v_y - w_y})$.
Using this, the edges can be defined as $E = \set{(v, w) \mid \manhattan{v - w} = 1 \mid v, w \in V}$ \fda{the second $ \mid $ is misplaced}.
...
Injective implies no two vertices in $V$ can be occupied at once, while $\bot$ signifies an empty vertex, so there will always be exactly $(\nngrid - N)$ empty squares in the grid. \fda{Rephrase: injectivity implies}

The \emph{inverse} of a configuration $\iconf{}{r} = (x, y), r \in R$ is the \emph{position} of a robot \fda{You did not defined the set $R$ before?}. 
The robots move synchronously and in parallel, a maximum of 1 square \fda{edge?} per step:
...
A \emph{schedule} is a series \fda{maybe sequence?} of transformation steps $\conf{1} \rightarrow \conf{2} \rightarrow \cdots \rightarrow \conf{k}$. 
...
\begin{definition}\label{def:motion_planning_problem}
	...
	the \emph{motion planning problem} is \fda{asks?} to find a \emph{schedule} that transforms $\conf{s}$ into $\conf{t}$.
\end{definition}
...
\begin{definition}\label{def:optimality}
	A schedule $S_\text{opt}$ is \emph{optimal} by some metric $f : S \rightarrow \mathbb{R}$ iff $f(S) \geq f(S_\text{opt})$ for all valid schedules $S$. \fda{maybe define metric?}
\end{definition}


\begin{definition}
	Let the makespan of a schedule be such a metric: $M : S \rightarrow \text{\# steps in } S$.
	The \emph{minimum makespan motion planning problem} is to find an \emph{optimal} schedule to \cref{def:motion_planning_problem} with respect to the makespan $M$ of that schedule. \fda{Rewrite as $\text{\# steps in } S$ is not standard notation.}
\end{definition}
...
\begin{definition}
	Let a schedule with makespan $M$ have the aforementioned lower bound as \fda{Maybe ``, denoted by''} $L$.
