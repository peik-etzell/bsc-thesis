...
By subdividing the workspace into fairly large but \(O(d)\) sized tiles, any transformations that is within such a tile can be done in linear time w.r.t. \fda{adjust the spacing here and everywhere else} the maximum horizontal or vertical distance, due to the previous findings with Rotatesort. 
